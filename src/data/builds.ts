...
export const builds: Build[] = [
  {
    ...
    description: "一个虚拟花园，你可以在里面培育像素艺术宠物。使用纯 HTML/CSS/JS 构建。",
    isPublished: true,
...
function water() {
  ...
  },
  {
    ...
    description: "经典贪吃蛇游戏的霓虹风格版本。使用方向键游玩。",
    isPublished: true,
...
setInterval(loop,150);`,
  ...
  },
  {
    ...
    description: "一个配有舒缓渐变背景的番茄钟计时器。保持专注，享受温馨。",
    isPublished: true,
...
function toggle(){ if(!running){running=true; document.getElementById('btn').textContent='Pause'; t=setInterval(tick,1000);} else {running=false; clearInterval(t); document.getElementById('btn').textContent='Resume';} }`,
  ...
  },
  {
    ...
    description: "在 16x16 网格上绘制像素艺术。将你的杰作导出为 ASCII。",
    isPublished: true,
