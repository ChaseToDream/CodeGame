...
export default function BuildDetailPage() {
  ...
  const previewDoc = useMemo(() => {
    const html = build.files.find((f) => f.language === "html")?.content ?? "";
    const css = build.files.find((f) => f.language === "css")?.content ?? "";
    const js = build.files.find((f) => f.language === "js")?.content ?? "";
    let doc = html
      .replace(/<link[^>]*href=["']style\.css["'][^>]*>/gi, `<style>${css}</style>`)
      .replace(/<script[^>]*src=["']script\.js["'][^>]*><\/script>/gi, `<script>${js}<\/script>`);
    if (!/<style>/i.test(doc) && css) doc = doc.replace(/<\/head>/i, `<style>${css}</style></head>`);
    if (!/<script>/i.test(doc) && js) doc = doc.replace(/<\/body>/i, `<script>${js}<\/script></body>`);
    return doc;
  }, [build]);
