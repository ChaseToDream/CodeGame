...
export default function SettingsPage() {
  ...
  if (!user) {
    return <div className="mx-auto max-w-3xl px-4 py-20 text-center text-muted">Loading...</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
      <header className="mb-8">
        ...
        <p className="text-muted mt-1 text-sm">Manage your profile, account, and preferences.</p>
      </header>
      ...
      {activeTab === "profile" && (
        <div className="space-y-6">
          <section className="rounded-xl border border-rule bg-bg2 p-5">
            <h2 className="font-outfit text-lg font-bold mb-4">Avatar</h2>
          ...
          <section className="rounded-xl border border-rule bg-bg2 p-5 space-y-4">
            <Field label="Username">
              ...
              <p className="text-[11px] text-muted mt-1">
                Your public handle. Visible at <code className="text-accent3">/u/{username || "username"}</code>
              </p>
            </Field>
      ...
      {activeTab === "account" && (
        <div className="space-y-6">
          <section className="rounded-xl border border-rule bg-bg2 p-5 space-y-4">
            <h2 className="font-outfit text-lg font-bold">Account info</h2>
            <Field label="Email">
              <input
                value={user.email}
                disabled
              ...
              />
              <p className="text-[11px] text-muted mt-1">Email cannot be changed in this demo.</p>
            </Field>
            <Field label="User ID">
              <input
                value={user.id}
          ...
          <section className="rounded-xl border border-rule bg-bg2 p-5">
            <h2 className="font-outfit text-lg font-bold mb-2">Danger zone</h2>
            <p className="text-sm text-muted mb-4">Log out of your account on this device.</p>
            <div className="flex flex-wrap gap-3">
              ...
              <button
                onClick={() => {
                  if (confirm("Reset all local progress, builds, and posts? This cannot be undone.")) {
      ...
      {activeTab === "notifications" && (
        <div className="space-y-6">
          <section className="rounded-xl border border-rule bg-bg2 p-5 space-y-4">
            <h2 className="font-outfit text-lg font-bold">Preferences</h2>
            <Toggle label="Email when I earn a new badge" defaultOn />
            <Toggle label="Email when someone replies to my post" defaultOn />
            <Toggle label="Weekly streak digest" defaultOn={false} />
            <Toggle label="Product updates & news" defaultOn />
            <p className="text-[11px] text-muted pt-2 border-t border-rule">
              Notifications are mocked in this demo. No emails will actually be sent.
            </p>
          </section>
        </div>
      )}
